# Tools

## Apps

- [Mimestream | A native macOS email client for Gmail](https://mimestream.com/)
- [Things. Your to-do list for Mac & iOS](https://culturedcode.com/things/)
- [Telegram for macOS](https://macos.telegram.org/)
- [Reeder 5 - RSS Client](https://reederapp.com/)
- [Monodraw - plain text diagrams](https://monodraw.helftone.com/)
- [OmniGraffle - Visual Communication Software To Make Pro Diagrams](https://www.omnigroup.com/omnigraffle)

## Developer tools

- [PyCharm: the Python IDE for Professional Developers](https://www.jetbrains.com/pycharm/)
- [Fork - a fast and friendly git client](https://fork.dev/)
- [Postico – a modern PostgreSQL client](https://eggerapps.at/postico/)
- [Dash for macOS - API Documentation Browser](https://kapeli.com/dash)

## Utilities

- [1Password | Password Manager](https://1password.com/)
- [Rectangle - Window manager](https://rectangleapp.com/)
- [Pastebot - Clipboard history](https://tapbots.com/pastebot/)
- [Arq: Cloud backup](https://www.arqbackup.com/)
- [SoundSource: A Superior Sound Control](https://rogueamoeba.com/soundsource/)
- [MonitorControl: Control your external monitor brightness & volume](https://github.com/MonitorControl/MonitorControl)
- [safari-markdown-linker: A handy Safari extension that helps user copying URL as markdown format](https://github.com/horimislime/safari-markdown-linker)
- [Dark Reader - Dark mode everywhere](https://darkreader.org/)
- [Nightfall: A menu bar utility for toggling dark mode](https://github.com/r-thomson/Nightfall)
- [iStat Menus](https://bjango.com/mac/istatmenus/)
